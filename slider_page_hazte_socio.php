<div class="swiper page-slider">
    <div class="swiper-wrapper wrapper">
        <div class="swiper-slide slide">
            <div class="image ahorro">
                    <img class="banner_fondo" src="./img/haztesocio/base.jpg"/>
                    <h1 class="banner_texto animate__animated animate__fadeInDown">
                    Tu cooperativa,
                    </h1>
                    <h1 class="banner_texto subtitulo animate__animated  animate__fadeInUp">
                    47 años creciendo juntos.
                    </h1>
            </div>
            
        </div>
    </div>
    <!-- <div class="swiper-button-next"></div>
    <div class="swiper-button-prev"></div> -->
    <!-- <div class="swiper-pagination"></div> -->
    <div class="accesos-det">
        <div class="notop-det">
            <div style="border-bottom: #fff 1px solid;">
                <a id="creditos">
                    <img src="img/icon_creditos.png" alt="Créditos">
                    <span> CRÉDITOS</span>  
                    <div class="fondo"></div>                       
                </a>
                <a id="ahorros">
                    <img src="img/icon_ahorros.png" alt="Ahorros">
                    <span> AHORROS</span>
                </a>
                <a id="servicio">
                    <img src="img/icon_prevision_social.png" alt="Servicios">
                    <span> SERVICIOS</span>
                </a>
                <a href="promociones.php">
                    <img src="img/icon_campañas.png" alt="Campañas">
                    <span> CAMPAÑAS</span>
                </a>
                <a >
                    <img src="img/icon_noticias.png" alt="Noticias">
                    <span> NOTICIAS</span>
                </a>
            </div>            
        </div>
    </div>
</div>